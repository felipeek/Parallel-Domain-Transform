	...
	r32* image_result)
{
	...
	for (s32 j = 0; j < parallelism_level_x; ++j)
	{
		...
		for (s32 i = 0; i < parallelism_level_y; ++i)
		{
			...
			(thread_informations + i * parallelism_level_x + j)->last_prologues = last_prologues;
...
extern void calculate_incomplete_prologues(const r32* image_bytes,
	...
	s32 number_of_threads)
{
	Thread_Incomplete_Prologue_Information* thread_informations = (Thread_Incomplete_Prologue_Information*)alloc_memory(sizeof(Thread_Incomplete_Prologue_Information)
		* parallelism_level_x * parallelism_level_y);
	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
		for (s32 j = 0; j < parallelism_level_x; ++j)
		{
			...
			(thread_informations + i * parallelism_level_x + j)->rf_table = rf_table;
			(thread_informations + i * parallelism_level_x + j)->iteration = iteration;
			(thread_informations + i * parallelism_level_x + j)->last_prologue_contribution = (r32*)alloc_memory(sizeof(r32)
				* (thread_informations + i * parallelism_level_x + j)->block->width);
			(thread_informations + i * parallelism_level_x + j)->incomplete_prologue = (r32*)alloc_memory(sizeof(r32)
	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
	{
		for (s32 j = 0; j < parallelism_level_x; ++j)
		{
			active_threads[num_active_threads++] = create_thread(fill_incomplete_prologues_thread_proc, thread_informations + i * parallelism_level_x + j);
	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
	{
		for (s32 j = 0; j < parallelism_level_x; ++j)
		{
			*(prologues + i * parallelism_level_x + j) = (thread_informations + i * parallelism_level_x + j)->incomplete_prologue;
			*(last_prologue_contributions + i * parallelism_level_x + j) = (thread_informations + i * parallelism_level_x + j)->last_prologue_contribution;
		}
	}

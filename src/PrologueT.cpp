...
static Thread_Proc_Return_Type _stdcall fill_blocks_from_prologuesT_thread_proc(void* thread_information)
{
	...
	Thread_Handler* active_threads_memory)
{
	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
	{
		...
		for (s32 j = 0; j < parallelism_level_x; ++j)
		{
			...
			thread_informations_memory[i * parallelism_level_x + j].domain_transforms = horizontal_domain_transforms;
			...
			thread_informations_memory[i * parallelism_level_x + j].last_prologuesT = last_prologuesT;
	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
	{
		for (s32 j = 0; j < parallelism_level_x; ++j)
		{
			active_threads_memory[num_active_threads++] = create_thread(fill_blocks_from_prologuesT_thread_proc, thread_informations_memory + i * parallelism_level_x + j);
...
static Thread_Proc_Return_Type _stdcall fill_incomplete_prologuesT_thread_proc(void* thread_information)
{
	...
	Thread_Handler* active_threads_memory)
{
	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
		for (s32 j = 0; j < parallelism_level_x; ++j)
		{
			...
			thread_informations_memory[i * parallelism_level_x + j].rf_table = rf_table;
			thread_informations_memory[i * parallelism_level_x + j].iteration = iteration;
			thread_informations_memory[i * parallelism_level_x + j].last_prologueT_contribution = last_prologueT_contributions[i * parallelism_level_x + j];
			thread_informations_memory[i * parallelism_level_x + j].incomplete_prologueT = prologuesT[i * parallelism_level_x + j];
		}
	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
	{
		for (s32 j = 0; j < parallelism_level_x; ++j)
		{
			active_threads_memory[num_active_threads++] = create_thread(fill_incomplete_prologuesT_thread_proc, thread_informations_memory + i * parallelism_level_x + j);

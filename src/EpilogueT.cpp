	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
	{
		for (s32 j = parallelism_level_x - 1; j >= 0; --j)
			...
			for (s32 k = 0; k < (image_blocks + i * parallelism_level_x + j)->height; ++k)
			{
				...
				{
					last_epilogueT = (R32_Color){ 0.0f, 0.0f, 0.0f };
				}
		...
		else
		{
			last_pixel = (R32_Color){
				block_information->image_bytes[j * block_information->image_information->width *
					block_information->image_information->channels + (block_information->block->x + block_information->block->width - 1) * block_information->image_information->channels],
				block_information->image_bytes[j * block_information->image_information->width *
					block_information->image_information->channels + (block_information->block->x + block_information->block->width - 1) * block_information->image_information->channels + 1],
				block_information->image_bytes[j * block_information->image_information->width *
					block_information->image_information->channels + (block_information->block->x + block_information->block->width - 1) * block_information->image_information->channels + 2],
			};
		}

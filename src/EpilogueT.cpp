	...
	u8* image_result)
{
	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
	{
		...
		for (s32 j = parallelism_level_x - 1; j >= 0; --j)
		{
			...
			(thread_informations + i * parallelism_level_x + j)->last_epiloguesT = last_epiloguesT;
...
extern void calculate_incomplete_epiloguesT(const u8* image_bytes,
	...
	s32 number_of_threads)
{
	Thread_Incomplete_EpilogueT_Information* thread_informations = (Thread_Incomplete_EpilogueT_Information*)alloc_memory(sizeof(Thread_Incomplete_EpilogueT_Information)
		* parallelism_level_x * parallelism_level_y);
	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
		for (s32 j = 0; j < parallelism_level_x; ++j)
		{
			...
			(thread_informations + i * parallelism_level_x + j)->rf_table = rf_table;
			(thread_informations + i * parallelism_level_x + j)->iteration = iteration;
			(thread_informations + i * parallelism_level_x + j)->last_epilogueT_contribution = (r32*)alloc_memory(sizeof(r32)
				* (thread_informations + i * parallelism_level_x + j)->block->height);
			(thread_informations + i * parallelism_level_x + j)->incomplete_epilogueT = (r32*)alloc_memory(sizeof(r32)
	...
	for (s32 i = 0; i < parallelism_level_y; ++i)
	{
		for (s32 j = 0; j < parallelism_level_x; ++j)
		{
			*(epiloguesT + i * parallelism_level_x + j) = (thread_informations + i * parallelism_level_x + j)->incomplete_epilogueT;
			*(last_epilogueT_contributions + i * parallelism_level_x + j) = (thread_informations + i * parallelism_level_x + j)->last_epilogueT_contribution;
		}
	}
